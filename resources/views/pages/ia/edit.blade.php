                    ...
                    @component('components/edits/mou_moa_ia')
                        ...
                        @slot('pengusul')
                            @forelse ($pengusul as $item)
                                <option daerah="{{ $item->negara->region }}" negara="{{ $item->negara->nama }}"
                                    provinsi="@if ($item->provinsi == null) {{ $item->provinsi_id }}
                                @else {{ $item->provinsi->nama }} @endif"
                                    kota="@if ($item->kota == null) {{ $item->kota_id }}
                                @else {{ $item->kota->nama }} @endif"
                                    kecamatan="@if ($item->kecamatan == null) {{ $item->kecamatan_id }}
                                @else {{ $item->kecamatan->nama }} @endif"
                                    kelurahan="@if ($item->kelurahan == null) {{ $item->kelurahan_id }}
                                @else {{ $item->kelurahan->nama }} @endif"
                                    latitude="{{ $item->latitude }}" longitude="{{ $item->longitude }}"
                                    alamat="{{ $item->alamat }}" value="{{ $item->id }}"
                                    @if (old('pengusul_id', $ia->pengusul->id) == $item->id) selected @endif>{{ $item->nama }}</option>
                            @empty
                        ...
                        @slot('nomor_moa_pengusul')
                            @forelse ($nomor_moa_pengusul as $item)
                                @if ($item->mou)
                                    <option
                                        nomor_mou_pengusul="{{ $item->mou->nomor_mou_pengusul }} - {{ $item->mou->pengusul->nama }}"
                                        value="{{ $item->id }}" @if (old('nomor_moa_pengusul', $ia->moa_id) == $item->id) selected @endif>
                                        {{ $item->nomor_moa_pengusul }} - {{ $item->pengusul->nama }}</option>
                                @else
                                    <option nomor_mou_pengusul="-" value="{{ $item->id }}"
                                        @if (old('nomor_moa_pengusul', $ia->moa_id) == $item->id) selected @endif>{{ $item->nomor_moa_pengusul }} -
                                        {{ $item->pengusul->nama }}</option>
                                @endif
                            @empty
                                <option value="">Tidak ada data</option>
                            @endforelse
