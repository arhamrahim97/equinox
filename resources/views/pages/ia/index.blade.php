...
@section('content')
    <section>
        @if (in_array(Auth::user()->role, ['Admin', 'Fakultas', 'Pascasarjana', 'PSDKU', 'LPPM', 'Prodi', 'Unit Kerja']))
            <div class="row mb-3">
                <div class="col-12">
        ...
        <form id="upload_tambahan" class="form_modal_change" method="POST" enctype="multipart/form-data" action="#">
            @csrf
            <div class="modal fade" id="upload-tambahan" tabindex="-1" role="dialog" aria-labelledby="upload-tambahan"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-dark" data-dismiss="modal"><i class="fas fa-times"></i>
                                {{ __('pages/ia/index.tutup') }}</button>
    ...
    <script>
        function showModalFileTambahan(id, jenis_upload) {
            if (jenis_upload == 'laporan_pelaksanaan') {
                ...
                $('#modalTitle').html(
                    "{{ __('pages/ia/index.upload') }} {{ __('components/form_mou_moa_ia.surat_tugas') }}")
            }
            $('#ia_id_').val(id)
