...
@section('title-tab')
| {{__('pages/master/kecamatan.title')}}
@endsection
...
@section('subTitle')
{{__('pages/master/kecamatan.subTitle')}}
@endsection
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{__('pages/master/kecamatan.modalTitleTambah')}}</h5>
            ...
            <div class="modal-body">
                <form id="form-tambah">
                    <div class="form-group">
                        <label for="nama">{{__('pages/master/kecamatan.title')}}</label>
                        <input type="text" class="form-control" id="nama"
                            placeholder="{{__('pages/master/kecamatan.placeHolderTambah')}}">
...
<div class="modal" tabindex="-1" role="dialog" id="modal-ubah">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{__('pages/master/kecamatan.modalTitleUbah')}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            ...
            success: function (data) {
                ...
                if ($.isEmptyObject(data.error)) {
                    swal("{{__('components/sweetalert.alertBerhasil')}}",
                        "{{__('components/sweetalert.msgTambahBerhasil',['nama' => __('pages/master/kecamatan.title')])}}", {
                            icon: "success",
                            buttons: {
                                confirm: {
    ...
    $('#form-ubah').on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if ($.isEmptyObject(data.error)) {
                    swal("{{__('components/sweetalert.alertBerhasil')}}",
                        "{{__('components/sweetalert.msgUbahBerhasil',['nama' => __('pages/master/kecamatan.title')])}}", {
                            icon: "success",
                            buttons: {
                                confirm: {
                                    className: 'btn btn-success'
