...
@section('title-tab')
| {{__('pages/master/kelurahan.title')}}
@endsection
...
@section('subTitle')
{{__('pages/master/kelurahan.subTitle')}}
@endsection
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{__('pages/master/kelurahan.modalTitleTambah')}}</h5>
            ...
            <div class="modal-body">
                <form id="form-tambah">
                    <div class="form-group">
                        <label for="nama">{{__('pages/master/kelurahan.title')}}</label>
                        <input type="text" class="form-control" id="nama"
                            placeholder="{{__('pages/master/kelurahan.placeHolderTambah')}}">
...
<div class="modal" tabindex="-1" role="dialog" id="modal-ubah">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{__('pages/master/kelurahan.modalTitleUbah')}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
            ...
            success: function (data) {
                ...
                if ($.isEmptyObject(data.error)) {
                    swal("{{__('components/sweetAlert.alertBerhasil')}}",
                        "{{__('components/sweetAlert.msgTambahBerhasil',['nama' => __('pages/master/kelurahan.title')])}}", {
                            icon: "success",
                            buttons: {
                                confirm: {
    ...
    $('#form-ubah').on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if ($.isEmptyObject(data.error)) {
                    swal("{{__('components/sweetAlert.alertBerhasil')}}",
                        "{{__('components/sweetAlert.msgUbahBerhasil',['nama' => __('pages/master/kelurahan.title')])}}", {
                            icon: "success",
                            buttons: {
                                confirm: {
                                    className: 'btn btn-success'
    ...
    function edit(id) {
        $.ajax({
            url: "/kelurahan/{{$kecamatan}}/" + id + "/edit",
            type: "GET",
            data: {
                id: id
            },
