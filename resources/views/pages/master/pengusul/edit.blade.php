                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <textarea class="form-control" id="alamat" rows="3"
                                    placeholder="{{__('pages/master/pengusul.placeholderAlamat')}}">{{$pengusul->alamat}}</textarea>
...
<script>
    ...
    function setValue() {
        $('#region').val('{{$pengusul->region}}').trigger('change');

        setTimeout(
            function () {
                $('#negara').val('{{$pengusul->negara_id}}').trigger('change');
            }, 1000
        );

        setTimeout(
            function () {
                $('#provinsi').val('{{$pengusul->provinsi_id}}').trigger('change');
            }, 2000
        );

        setTimeout(
            function () {
                $('#kota').val('{{$pengusul->kota_id}}').trigger('change');
            }, 3000
        );

        setTimeout(
            function () {
                $('#kecamatan').val('{{$pengusul->kecamatan_id}}').trigger('change');
            }, 4000
        );

        setTimeout(
            function () {
                $('#kelurahan').val('{{$pengusul->kelurahan_id}}').trigger('change');
            }, 5000
        );
    }
...
<script>
    var map = L.map("peta").setView(["{{$pengusul->latitude}}", "{{$pengusul->longitude}}"], 15);
    ...
    var marker = L.marker(["{{$pengusul->latitude}}", "{{$pengusul->longitude}}"]).addTo(map);
